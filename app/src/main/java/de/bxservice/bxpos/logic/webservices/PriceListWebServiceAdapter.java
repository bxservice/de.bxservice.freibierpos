...
import de.bxservice.bxpos.logic.model.PriceList;

/**
 * Created by Diego Ruiz on 9/11/15.
 */
public class PriceListWebServiceAdapter extends AbstractWSObject{
    ...
    public PriceListWebServiceAdapter(Context ctx) {
        super(ctx);
    }
            ...
            if ( response.getStatus() == Enums.WebServiceResponseStatus.Error ) {
                ...
                for (int i = 0; i < response.getDataSet().getRowsCount(); i++) {
                    ...
                    for (int j = 0; j < response.getDataSet().getRow(i).getFieldsCount(); j++) {
                        ...
                        else if ( PriceList.M_PriceList.equalsIgnoreCase(field.getColumn()) )
